...
To troubleshoot OpenClaw, use the /debug command in chat for runtime config overrides and run the gateway in verbose mode for detailed logs. For raw stream logging, use pnpm gateway:watch --raw-stream.

### 补充 2

- **Debugging - OpenClaw Docs** (relevance: 75%)
  https://docs.openclaw.ai/help/debugging
  * [Dev profile + dev gateway (—dev)](#dev-profile-%2B-dev-gateway-%E2%80%94dev). * [Raw stream logging (OpenClaw)](#raw-stream-logging-openclaw). Use `/debug` in chat to set **runtime-only** config overrides

### 补充 3

- **[Bug]: Logging level config (OPENCLAW_LOG_LEVEL ... - GitHub** (relevance: 74%)
  https://github.com/openclaw/openclaw/issues/29448
  DEBUG logs continue to be written to /tmp/openclaw/openclaw-YYYY-MM-DD.log regardless of log level configuration. Log shows logLevelName:DEBUG

### 补充 4

- **OpenClaw Server Log Management and Analysis Tools** (relevance: 71%)
  https://www.tencentcloud.com/techpedia/140855
  Technology Encyclopedia Home >OpenClaw Server Log Management and Analysis Tools. # OpenClaw Server Log Management and Analysis Tools. For OpenClaw server log management and anal

### 补充 5

- **Logging - OpenClaw** (relevance: 71%)
  https://docs.openclaw.ai/gateway/logging
  [Skip to main content](https://docs.openclaw.ai/gateway/logging#content-area). [Get started](https://docs.openclaw.ai/)[Install](
